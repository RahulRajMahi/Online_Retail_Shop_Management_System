...
emp_mobile_no number(10) constraint emp_mbl_chk check(emp_mobile_no > 0) not null,
...
cust_type char(9) default 'ARBITRARY' constraint cust_type_chk check(cust_type in('PERMANENT','ARBITRARY')) not null
);
...
cust_type char(9) default 'ARBITRARY' constraint cust_type_chek check(cust_type in('PERMANENT','ARBITRARY')) not null,
...
complain_status varchar2(8) constraint cmpln_status_chk check(complain_status in('RESOLVED','UNRESOLVED')) not null,
